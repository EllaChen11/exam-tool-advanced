...
if uploaded_file:
    ...
    if st.button("分析并生成报告"):
        ...
        if stu.empty:
            ...
            sns.lineplot(x="日期", y="分数变化", data=median_df, marker='s', linestyle='--', ax=ax2, label="班级中位数变化")
            if my_font:
            ...
            def explain_diff(x):
                if x > 0:
                    return "高于班级中位数"  # 高于中位数表示学生成绩比班级中位数高
                elif x < 0:
                    return "低于班级中位数"
                else:
                    return "等于班级中位数"
            ...
            compare_df["总分标准差"] = stu["总分"].std()  # 学生成绩波动
            compare_df["班级中位数标准差"] = df.groupby("日期")["总分"].std().mean()  # 班级波动

            st.subheader("📋 历次成绩对比班级中位数")
            st.dataframe(compare_df[["日期", "总分_学生", "总分_班级中位数", 
                                     "与班级中位数差", "解释", "总分标准差", "班级中位数标准差"]])
            ...
            # 保存图表到图片再写入 PDF
            fig1_buf = io.BytesIO()
            fig1.savefig(fig1_buf, format="PNG", bbox_inches='tight')
            fig1_buf.seek(0)
            pdf.image(fig1_buf, x=10, w=180)
            pdf.ln(5)

            fig2_buf = io.BytesIO()
            fig2.savefig(fig2_buf, format="PNG", bbox_inches='tight')
            fig2_buf.seek(0)
            pdf.image(fig2_buf, x=10, w=180)
            pdf.ln(5)
            ...
            for idx, row in compare_df.iterrows():
                text = (f"{row['日期'].strftime('%Y-%m-%d')} 学生:{row['总分_学生']} "
                        ...
                        f"学生标准差:{row['总分标准差']:.2f} "
                        f"班级标准差:{row['班级中位数标准差']:.2f}")
                # 按行分割文字，避免过长报错
                max_len = 80
                for i in range(0, len(text), max_len):
                    pdf.multi_cell(0, 6, text[i:i+max_len])
            pdf_output = f"{student_name}_成绩分析报告.pdf"
            pdf.output(pdf_output)

            st.success(f"✅ PDF 报告生成成功: {pdf_output}")
            st.download_button("下载 PDF 报告", data=open(pdf_output, "rb").read(), file_name=pdf_output)
